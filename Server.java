	...
	public static void fileChunks(File f) throws IOException {
		...
		try {
			...
			while ((full = B.read(buf)) > 0){
				...
				File newFile = new File(f.getParent()+"//Chunks",name);
         
				FileOutputStream out = new FileOutputStream(newFile);
				out.write(buf, 0, full);
				Size = Size + newFile.length();
				
				F.add(newFile);
			}
			System.out.println("Size of the file given: "+Size);
			System.out.println("No. of chunks"+ChunkCnt);
		}catch(IOException e){}
	}
